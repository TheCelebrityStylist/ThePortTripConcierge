...
type Props = {
  hasPlan: boolean;
  onPrimary: () => void;
  onAction: (key: "save" | "export" | "duplicate" | "share") => void;
};

export default function PlannerHeader({ hasPlan, onPrimary, onAction }: Props) {
  return (
    <header className="sticky top-12 z-30 border-b border-white/10 bg-slate-950/95 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-3 py-3 sm:px-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Ship-safe itinerary builder</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <details className="relative">
            ...
            <div className="absolute right-0 mt-2 w-40 rounded-lg border border-white/10 bg-slate-900 p-1 text-sm">
              {(["Save", "Export", "Duplicate", "Share"] as const).map((item) => (
                <button key={item} className="block w-full rounded px-2 py-1 text-left hover:bg-white/10" onClick={() => onAction(item.toLowerCase() as "save" | "export" | "duplicate" | "share")}>{item}</button>
              ))}
            </div>
          </details>
        </div>
