  ...
  return (
    <header className="rounded-3xl border border-cyan-300/25 bg-gradient-to-br from-slate-900 via-slate-900 to-cyan-950/75 p-6 md:p-8">
      <div className="flex flex-wrap gap-2 text-xs">
        ...
        <span className="rounded-full bg-cyan-500/20 px-2 py-1">{article.timeInPortModel}</span>
        <span className="rounded-full bg-violet-500/20 px-2 py-1">{article.tenderOrDock}</span>
        <span className="rounded-full bg-amber-500/20 px-2 py-1">{article.difficulty}</span>
        <span className="rounded-full bg-emerald-500/20 px-2 py-1">{article.dataConfidence === "estimated" ? "Estimated" : "Verified"}</span>
      </div>
