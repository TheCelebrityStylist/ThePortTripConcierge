...
export async function researchPortContext(portSlug?: string): Promise<PortResearchContext> {
  ...
  try {
    const res = await fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(query)}`, { cache: "no-store" });
    if (!res.ok) throw new Error("wiki fetch failed");
    const data = (await res.json()) as { extract?: string };
    const summary = data.extract?.split(".").slice(0, 2).join(".")?.trim();
